/* 
===============================================================================
uArm Metal - Calibration Example

IMPORTANT:
Before compiling this example, install the official uArm Arduino library:
https://github.com/uArm-Developer/UArmForArduino

The library can be downloaded from GitHub and added to the Arduino IDE
using Sketch > Include Library > Add .ZIP Library, or by copying it into
...
1. Wait until you press the D7 button to start.
2. Automatically detects:
   - SERVO_LEFT maximum angle
   - SERVO_RIGHT minimum angle
3. Collects hundreds of points for each servo to calculate the linear regression.
4. Waits for you to manually move the uArm to the correct position to calibrate
   SERVO_ROT. Refer to:
...
void myattachServo(byte servo_number) {
  switch (servo_number) {
    case SERVO_ROT_NUM:
...
void mydetachServo(byte servo_number) {
  switch (servo_number) {
    case SERVO_ROT_NUM:
      rotServo.detach();
      break;
